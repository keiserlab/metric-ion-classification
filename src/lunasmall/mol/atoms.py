...
class ChimeraXAtom:
    def __init__(self, atm, invariants=None):
        self._atom = atm
...
class ExtendedAtom:
    ...
    def full_id(self):
        ...
        return (self._atom.residue.name, self._atom.residue.number, self._atom.name)
    ...
    def full_atom_name(self):
        ...
        return '/'.join([str(x) for x in self.full_id])
        full_atom_name = "%s/%s/%s" % self.full_id[0:3]
    ...
    def __lt__(self, a2):
        ...
        full_id1 = self.full_id[0:2] + (self.parent.number, ) + self.full_id[4:]
        full_id2 = a2.full_id[0:2] + (a2.parent.number, ) + a2.full_id[4:]
